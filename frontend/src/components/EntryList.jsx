...
export default function EntryList({ records, tamperedIds }) {
  ...
  return (
    <section id="explorer" className="w-full max-w-6xl mx-auto px-4 sm:px-8 py-10 border-t border-[#1a1a1a]">
      ...
      <div className="bg-[#0a0a0a] border border-[#242424] rounded-[8px] p-4 mb-6 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        {/* Search Input */}
        <div className="relative flex-1">
          <FiSearch className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#5a5654] w-3.5 h-3.5" />
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by record ID, type, or SHA-256 hash..."
            className="w-full bg-[#080808] border border-[#1f1f1f] focus:border-[#c9793f] rounded-[6px] pl-9 pr-3 py-2 font-mono text-xs text-[#f0ece9] focus:outline-none transition-colors"
          />
        </div>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0">
          ...
          {systems.map((sys) => (
            <button
              ...
              className={`font-mono text-[11px] px-2.5 py-1 rounded-[4px] whitespace-nowrap transition-colors ${
                selectedSystem === sys
          ...
          <button
            ...
            className="font-mono text-[11px] text-[#8a8480] hover:text-[#f0ece9] px-2.5 py-1 rounded bg-[#111111] hover:bg-[#161616] border border-[#1f1f1f] flex items-center gap-1 ml-auto"
      ...
      {filteredRecords.length === 0 ? (
        <div className="bg-[#0a0a0a] border border-[#1f1f1f] rounded-[10px] py-16 text-center">
